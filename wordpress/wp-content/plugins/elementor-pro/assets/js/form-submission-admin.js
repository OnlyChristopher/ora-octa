/*! elementor-pro - v3.8.1 - 08-11-2022 */
/******/ (() => { // webpackBootstrap
...
function useRestDataList(command) {
  ...
  const queryArgs = [...defaultQueryArgs, ...options.allowedFilters],
        [fetchOptions, setFetchOptions] = useState(false),
        [{
    data,
    meta
  ...
  const [fetchFetchOptions, {
    status: fetchOptionsStatus
    ...
    return $e.data.options(command, {}, {
      signal: abortController.signal
    }).then(result => normalizeFetchOptionsResult(result.data, options.allowedFilters)).then(normalizedResult => setFetchOptions(normalizedResult));
  }, [command]); // The fetch data action.

  const [fetchData, {
    status: fetchDataStatus
  }] = (0, _useDataAction.default)((args, _ref2) => {
    let {
      abortController
    } = _ref2;
  ...
  }, [command, flatQuery]); // Runs on render only and fetch all the "fetch options".
...
function normalizeFetchOptionsResult(data, allowedFilters) {
  const endpoint = data.endpoints.find(dataEndpoint => dataEndpoint.methods.includes('GET'));

  if (!endpoint) {
    return {};
  }

  return Object.entries(endpoint.args).reduce((current, _ref4) => {
...
const FORM_FILTER_KEY = 'form';
const REFERER_FILTER_KEY = 'referer';
const AFTER_FILTER_KEY = 'after';
const BEFORE_FILTER_KEY = 'before';
...
function Index() {
  ...
  const isTrashFilterOn = useMemo(() => 'trash' === filter[STATUS_FILTER_KEY], [filter[STATUS_FILTER_KEY]]);
  ...
  if (_useDataAction.STATUS_SUCCESS !== fetchOptionsStatus) {
  ...
  return /*#__PURE__*/_react.default.createElement(_react.default.Fragment, null, /*#__PURE__*/_react.default.createElement("div", null, ((_fetchOptions$filter = fetchOptions.filter) === null || _fetchOptions$filter === void 0 ? void 0 : _fetchOptions$filter[SEARCH_FILTER_KEY]) && /*#__PURE__*/_react.default.createElement(_searchBox.default, {
    ...
    isSearching: !!filter[SEARCH_FILTER_KEY] && fetchDataStatus === _useDataAction.STATUS_LOADING
    ...
    options: fetchOptions.filter[STATUS_FILTER_KEY].enum.map(value => {
    ...
    onChange: _ref8 => {
      ...
      setFilter({ ...(after !== undefined ? {
          [AFTER_FILTER_KEY]: after
        } : {}),
        ...(before !== undefined ? {
          [BEFORE_FILTER_KEY]: before
        } : {})
      });
    },
